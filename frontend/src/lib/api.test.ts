...
describe("vehicle BOM API contract", () => {
  it("submits only editable fields, preserves child names, and allows clearing notes", () => {
    ...
    expect(payload).not.toHaveProperty("standard_description")
    expect(payload).not.toHaveProperty("from_date")
  ...
  it("does not silently turn an invalid vehicle count into one", () => {
    const payload = toSavePayload({ ...study, status: "Draft", projects: [{ project: "PROJ-1", vehicle_count: 0 }] })
